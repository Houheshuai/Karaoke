#ifndef POLYGON_INFO_H__
#define POLYGON_INFO_H__
...
public:
     ...
     * @endcode
     */
     std::vector<Vec2> trace(const Rect& rect, const float& threshold = 0.0);
...
protected:
    ...
    std::vector<Vec2> marchSquare(const Rect& rect, const Vec2& first, const float& threshold);
    ...
    Vec2 getPosFromIndex(const unsigned int& i){return Vec2(i%_width, i/_width);};

    std::vector<Vec2> rdp(std::vector<Vec2> v, const float& optimization);
