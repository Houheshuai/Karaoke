...
std::string JniHelper::getStringUTFCharsJNI(JNIEnv* env, jstring srcjStr)
{
	std::string utf8Str;
	const char * utf8Char = ( const char *)env->GetStringUTFChars(srcjStr, nullptr);
	utf8Str = std::string(utf8Char);
	env->ReleaseStringUTFChars(srcjStr, utf8Char);
	return utf8Str;
}

jstring JniHelper::newStringUTFJNI(JNIEnv* env, std::string utf8Str)
{
	jstring stringText = env->NewStringUTF((char*)utf8Str.c_str());
	return stringText;
}
