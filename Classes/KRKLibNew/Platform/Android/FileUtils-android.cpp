...
FileUtils* FileUtils::getInstance()
{
    if (s_sharedFileUtils == nullptr)
    {
        ...
        if(!s_sharedFileUtils->init())
        {
          ...
          //LOG("ERROR: Could not init CCFileUtilsAndroid");
        }
    }
    ...
    if (strFilePath[0] != '/')
    {
        ...
        if (FileUtilsAndroid::assetmanager) {
            ...
            if (aa)
            {
                ...
                // //LOG("[AssetManager] ... in APK %s, found = false!", strFilePath.c_str());
            }
        }
    }
    else
...
bool FileUtilsAndroid::isDirectoryExistInternal(const std::string& dirPath) const
{
    ...
    if (s[0] == '/')
    {
        //LOG("find in flash memory dirPath(%s)", s);
        struct stat st;
        if (stat(s, &st) == 0)
        {
...
Data FileUtilsAndroid::getData(const std::string& filename, bool forString)
{
    ...
    if (fullPath[0] != '/')
    {
        ...
        //LOGINFO("relative path = %s", relativePath.c_str());
    ...
    else
    {
        do
        {
            ...
            ////LOG("GETTING FILE ABSOLUTE DATA: %s", filename);
