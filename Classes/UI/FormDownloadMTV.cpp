...
const String FormDownloadMTV::EventNamespace("FormDownloadMTV");
const String FormDownloadMTV::WidgetTypeName("FormDownloadMTV");

//----------------------------------------------------------------------------//
FormDownloadMTV::FormDownloadMTV(const String& type, const String& name):
M3D_Form(type, name)
{
...
//----------------------------------------------------------------------------//
FormDownloadMTV::~FormDownloadMTV(void)
{

}

//----------------------------------------------------------------------------//
void FormDownloadMTV::constructWindow(WndRes_t *wndRes)
{
	M3D_Form::constructWindow(wndRes);
}

//----------------------------------------------------------------------------//
void FormDownloadMTV::destructWindow(void)
{
	M3D_Form::destructWindow();
}
	
//----------------------------------------------------------------------------//
void FormDownloadMTV::onShown(WindowEventArgs& e)
{
	M3D_Form::onShown(e);	
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::onHidden(WindowEventArgs& e)
{
	M3D_Form::onHidden(e);
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::onCreated(void)
{
	M3D_Form::onCreated();
		
	d_songlist = static_cast<M3D_PageListView *>(getChild(Res_FormDownloadMTV::SongList));
	d_songlist->setCircleEnable(false);
	d_songlist->setFocusBarResID(Res_FormDownloadMTV::FocusBar);
	d_songlist->onDataFill = CALLBACK_2(FormDownloadMTV::handleReloadData, this);
	d_songlist->onPageChange = CALLBACK_2(FormDownloadMTV::handlePageChange, this);
	d_songlist->setOnClickListener(CALLBACK_2(FormDownloadMTV::handleListClicked, this));
	d_songlist->setOnFocusChangeListener(CALLBACK_2(FormDownloadMTV::handleFocusChange, this));

	d_scrollbar = static_cast<M3D_ScrollBar*>(d_songlist->getChild(Res_FormDownloadMTV::ScrollBar));
	d_scrollbar->setPageValue(1);

	d_displaypic = static_cast<M3D_Label*>(getChild(Res_FormDownloadMTV::DisplayWindow)->getChild(Res_FormDownloadMTV::DisplayPic));
	d_searchlable = static_cast<M3D_Label*>(getChild(Res_FormDownloadMTV::T9));
	d_searchstr = static_cast<M3D_Label*>(d_searchlable->getChild(Res_FormDownloadMTV::T9string));
	d_singerName = static_cast<M3D_Label*>(getChild(Res_FormDownloadMTV::DisplayWindow)->getChild(Res_FormDownloadMTV::DisplayName));
	d_focusbar = static_cast<M3D_Label*>(getChild(Res_FormDownloadMTV::SongList)->getChild(Res_FormDownloadMTV::FocusBar));

	d_progcount = static_cast<M3D_Label*>(getChild(Res_FormDownloadMTV::ProgCount));
	d_count = static_cast<M3D_Label*>(d_progcount->getChild(Res_FormDownloadMTV::Count));
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::onDestroyed(void)
{	
	M3D_Form::onDestroyed();
}

//----------------------------------------------------------------------------//
void FormDownloadMTV::onActivated(ActivationEventArgs& e)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::onDeactivated(ActivationEventArgs& e)
{
	M3D_Form::onDeactivated(e);
}

//----------------------------------------------------------------------------//
void FormDownloadMTV::onActionStart(void)
{
	M3D_Form::onActionStart();
}

//----------------------------------------------------------------------------//
void FormDownloadMTV::onActionEnd(void)
{
	M3D_Form::onActionEnd();
}

//----------------------------------------------------------------------------//
void FormDownloadMTV::onTimeout(EventArgs& e)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::onCharacter(KeyEventArgs& e)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::initFormData(void)
{
...
void FormDownloadMTV::setFormData(int type, int subtype)
{
	switch(type)
	{
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::setupBinding(bool firsttime)
{
	ReqPhoneDB* reqDb = (ReqPhoneDB*)ReqDB::getSingletonPtr();
	assert(reqDb != NULL);
	...
	if(firsttime)
	{
		...
		handle->reqCmd.subType = mlog->getParamInt("SubType", REQDB_SUBTYPE_MTV);
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::handleReloadData(M3D_Item *item, int position)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::handlePageChange(int currentPage, int totalPage)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMTV::handleListClicked(M3D_Item *item, int position)
{		
		
}

void FormDownloadMTV::handleFocusChange(M3D_Item *item, int position)
{		
	refreshFocusInfo(true);
}

bool FormDownloadMTV::refreshFocusInfo(bool refreshSingerName)
{
...
void FormDownloadMTV::updateSinger(void)
{
	ReqPhoneDB* reqDb = (ReqPhoneDB *)(ReqPhoneDB::getSingletonPtr());
