...
//
// Filename : FormDownloadMovie.cpp
//																	
// Description: 
// 
//----------------------------------------------------------------------------//
...
#include "FormDownloadMovie.res.h"
...
const String FormDownloadMovie::EventNamespace("FormDownloadMovie");
const String FormDownloadMovie::WidgetTypeName("FormDownloadMovie");

//----------------------------------------------------------------------------//
FormDownloadMovie::FormDownloadMovie(const String& type, const String& name):
M3D_Form(type, name)
{
...
//----------------------------------------------------------------------------//
FormDownloadMovie::~FormDownloadMovie(void)
{

}

//----------------------------------------------------------------------------//
void FormDownloadMovie::constructWindow(WndRes_t *wndRes)
{
	M3D_Form::constructWindow(wndRes);
}

//----------------------------------------------------------------------------//
void FormDownloadMovie::destructWindow(void)
{
	M3D_Form::destructWindow();
}
	
//----------------------------------------------------------------------------//
void FormDownloadMovie::onShown(WindowEventArgs& e)
{
	M3D_Form::onShown(e);	
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::onHidden(WindowEventArgs& e)
{
	M3D_Form::onHidden(e);
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::onCreated(void)
{
	M3D_Form::onCreated();
		
	d_songlist = static_cast<M3D_PageListView *>(getChild(Res_FormDownloadMovie::SongList));
	d_songlist->setCircleEnable(false);
	d_songlist->setFocusBarResID(Res_FormDownloadMovie::FocusBar);
	d_songlist->onDataFill = CALLBACK_2(FormDownloadMovie::handleReloadData, this);
	d_songlist->onPageChange = CALLBACK_2(FormDownloadMovie::handlePageChange, this);
	d_songlist->setOnClickListener(CALLBACK_2(FormDownloadMovie::handleListClicked, this));
	d_songlist->setOnFocusChangeListener(CALLBACK_2(FormDownloadMovie::handleFocusChange, this));

	d_scrollbar = static_cast<M3D_ScrollBar*>(d_songlist->getChild(Res_FormDownloadMovie::ScrollBar));
	d_scrollbar->setPageValue(1);

	d_displaypic = static_cast<M3D_Label*>(getChild(Res_FormDownloadMovie::DisplayWindow)->getChild(Res_FormDownloadMovie::DisplayPic));
	d_searchlable = static_cast<M3D_Label*>(getChild(Res_FormDownloadMovie::T9));
	d_searchstr = static_cast<M3D_Label*>(d_searchlable->getChild(Res_FormDownloadMovie::T9string));
	d_singerName = static_cast<M3D_Label*>(getChild(Res_FormDownloadMovie::DisplayWindow)->getChild(Res_FormDownloadMovie::DisplayName));
	d_focusbar = static_cast<M3D_Label*>(getChild(Res_FormDownloadMovie::SongList)->getChild(Res_FormDownloadMovie::FocusBar));

	d_progcount = static_cast<M3D_Label*>(getChild(Res_FormDownloadMovie::ProgCount));
	d_count = static_cast<M3D_Label*>(d_progcount->getChild(Res_FormDownloadMovie::Count));
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::onDestroyed(void)
{	
	M3D_Form::onDestroyed();
}

//----------------------------------------------------------------------------//
void FormDownloadMovie::onActivated(ActivationEventArgs& e)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::onDeactivated(ActivationEventArgs& e)
{
	M3D_Form::onDeactivated(e);
}

//----------------------------------------------------------------------------//
void FormDownloadMovie::onActionStart(void)
{
	M3D_Form::onActionStart();
}

//----------------------------------------------------------------------------//
void FormDownloadMovie::onActionEnd(void)
{
	M3D_Form::onActionEnd();
}

//----------------------------------------------------------------------------//
void FormDownloadMovie::onTimeout(EventArgs& e)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::onCharacter(KeyEventArgs& e)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::initFormData(void)
{
...
void FormDownloadMovie::setFormData(int type, int subtype)
{
	switch(type)
	{
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::setupBinding(bool firsttime)
{
	ReqPhoneDB* reqDb = (ReqPhoneDB*)ReqDB::getSingletonPtr();
	assert(reqDb != NULL);
	...
	if(firsttime)
	{
		...
		handle->reqCmd.subType = mlog->getParamInt("SubType", REQDB_SUBTYPE_MOVIE);
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::handleReloadData(M3D_Item *item, int position)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::handlePageChange(int currentPage, int totalPage)
{
...
//----------------------------------------------------------------------------//
void FormDownloadMovie::handleListClicked(M3D_Item *item, int position)
{		
		
}

void FormDownloadMovie::handleFocusChange(M3D_Item *item, int position)
{		
	refreshFocusInfo(true);
}

bool FormDownloadMovie::refreshFocusInfo(bool refreshSingerName)
{
...
void FormDownloadMovie::updateSinger(void)
{
	ReqPhoneDB* reqDb = (ReqPhoneDB *)(ReqPhoneDB::getSingletonPtr());
