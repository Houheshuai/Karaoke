...
skinWidget::skinWidget(QWidget *parent) : baseDialog(parent)
{
    ...
    myPushButton *m_btn=new myPushButton(m_mainwid);//关闭按钮
    QLabel *label=new QLabel("主题皮肤与窗口调整",m_mainwid);
    m_btn->setGeometry(540,5,20,20);
    label->setGeometry(5,5,200,20);
    label->setStyleSheet("QLabel{font-size: 14px;color: white; font-family:微软雅黑}");
...
void skinWidget::initTopLayout()
{
    ...
    QLabel *label2=new QLabel(tr("皮肤"),contentwidget);
    label2->setFixedSize(100,24);
    label2->setStyleSheet("QLabel{font-family:微软雅黑}");

    m_btnAdjustWindow=new myPushButton(tr("恢复窗口默认状态"),contentwidget);
    m_btnAdjustWindow->setStyleSheet("QPushButton{border:NULL; font-family:微软雅黑; color: rgb(134,134,134);}"
...
void skinWidget::initBottomLayout()
{
    ...
    bottomwid->setStyleSheet("QWidget{background:white; font-family:微软雅黑}");
...
void skinContentWidget::UpdateSkin()//添加皮肤图片后更新列表
{
...
void skinWidgetContentItem::paintEvent(QPaintEvent *)
{
    ...
    if(m_isenter)
    {
        ...
        p.setFont(font);
        p.setPen(Qt::white);    //背景图上显示名称等信息
        p.drawText((width()-metric.width(m_maskText))/2,32,m_maskText);
        p.drawText((width()-metric.width("作者：侯贺帅"))/2,46,"作者：侯贺帅");
...
{
    this->setCursor(Qt::PointingHandCursor);    //光标变手指状
    m_isenter=true;
    update();
}
