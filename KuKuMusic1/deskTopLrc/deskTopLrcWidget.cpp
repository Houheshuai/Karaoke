...
void deskTopLrcWidget::paintEvent(QPaintEvent *)
{
    ...
    */
    ...
    else/*draw the original text here*/
    {
        ...
        int x=(width()-metrics.width("欢迎使用酷酷音乐"))/2;
        m_normalGradient.setStart(0, y);
        m_normalGradient.setFinalStop(0, y+metrics.height());

        painter.setPen(QColor(0, 0, 0, 200));
        painter.drawText(x+1,y+1,metrics.width("欢迎使用酷酷音乐"),metrics.height(),Qt::AlignLeft,"欢迎使用酷酷音乐");//左对齐

        painter.setPen(QPen(m_normalGradient,0));
        painter.drawText(x,y,metrics.width("欢迎使用酷酷音乐"),metrics.height(),Qt::AlignLeft,"欢迎使用酷酷音乐");//左对齐
    }
